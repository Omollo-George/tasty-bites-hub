...
# Import SSE emitter from primary views module if available
try:
    from .views import _emit_event
except Exception:
    def _emit_event(event_type, data):
        # fallback no-op if SSE isn't available in this module
        return None
...
def _get_token_from_request(request):
    ...
    return (
        ...
        or request.headers.get('X-STAFF-TOKEN')
    ...
    try:
        ...
        for activity in activities:
            ...
            if not table_label and order:
                table_label = order.table.number if order.table else ('Takeaway' if order.order_type == 'takeaway' else None)
            results.append({
...
def _get_period_dates(period_type: str, date_str: str, start_date_str: str | None = None, end_date_str: str | None = None):
    ...
    if period_type == 'day':
        ...
        try:
            start_date_val = datetime.datetime.strptime(start_date_str, '%Y-%m-%d').date() if start_date_str else selected_date
        except ValueError:
            start_date_val = selected_date
        try:
            end_date_val = datetime.datetime.strptime(end_date_str, '%Y-%m-%d').date() if end_date_str else selected_date
        except ValueError:
        ...
        label = f"Custom: {start_date_val.strftime('%Y-%m-%d')} to {end_date_val.strftime('%Y-%m-%d')}"
...
def _build_report_summary(start_date: datetime.datetime, end_date: datetime.datetime, range_label: str):
    ...
    # Build hourly sales from actual order creation timestamps to reflect real time order activity.
    hourly_map = {
        hour: {'hour': hour, 'label': f'{hour:02d}:00', 'orders': 0, 'revenue': 0.0}
        for hour in range(24)
    }

    for row in paid_orders.values('created_at', 'total_amount'):
        created_at = row.get('created_at')
        if created_at is None:
            continue
        local_dt = timezone.localtime(created_at)
        hour = local_dt.hour
        hourly_map[hour]['orders'] += 1
        hourly_map[hour]['revenue'] += float(row.get('total_amount') or 0)
    ...
    hourly = [
        {
            ...
            'label': entry['label'],
            'orders': entry['orders'],
            'revenue': _to_display_currency(entry['revenue']),
        }
...
def mark_table_free(request, table_id):
    """Marks a table as available (free) after payment confirmation."""
    if request.method != 'POST':
...
def get_active_pos_order(request):
    ...
    try:
        ...
        order = Order.objects.filter(
            table=table,
            status__in=['pending', 'preparing', 'sent_kitchen', 'ready', 'paid', 'bill_pending']
...
def create_pos_order(request):
    ...
    try:
        ...
        order = Order.objects.create(
            table=table,
            waiter=_get_staff_employee(request),
            waiter_name=_get_staff_name(request) or '',
        ...
        _log_staff_activity(
            ...
            {
                'order_id': order.order_id,
                'table': table.number if table else 'Counter',
...
def add_to_pos_order(request, order_id):
    ...
    try:
        ...
        _log_staff_activity(
            request,
            'Updated order with additional items',
            {
                'order_id': order.order_id,
                'added_items': [
                    {
...
def kds_queue(request):
    ...
    try:
        ...
        orders = Order.objects.filter(
            status__in=['preparing', 'pending', 'paid', 'sent_kitchen', 'bill_pending']
...
def cashier_pending_bills(request):
    ...
    try:
        bills = Order.objects.filter(status__in=['sent_kitchen', 'bill_pending', 'ready']).select_related('table').prefetch_related('items').order_by('-created_at')
...
def cashier_confirm_payment(request, order_id):
    ...
    try:
        ...
        if order.status not in ['sent_kitchen', 'bill_pending', 'ready']:
            return JsonResponse({'error': 'order_not_in_billable_state'}, status=400)
        
        payment_method = payload.get('payment_method', 'cash')
        
        response_data = {'ok': True, 'order': _serialize_order(order), 'payment_method': payment_method}

        # If M-Pesa, initiate STK push using provided mpesa_number or order.phone
        if payment_method == 'mpesa':
            ...
            # create a transaction linked to this order
            tx = Transaction.objects.create(
                phone=msisdn,
            ...
            # set order payment method to mpesa and keep status as bill_pending/payment_pending
            order.payment_method = 'mpesa'
            order.save()

            resp_json, status_code = _execute_stk_push(msisdn, order.total_amount, order.order_id, tx)

            # Return STK push response to frontend so it can poll/check status.
            response_data['mpesa'] = resp_json
            response_data['checkout_request_id'] = resp_json.get('CheckoutRequestID') if isinstance(resp_json, dict) else None
            response_data['status_code'] = status_code

            _log_staff_activity(
                request,
                'Initiated M-Pesa STK push',
                {
                    'order_id': order.order_id,
                    'mpesa_number': msisdn,
                    'stk_response': resp_json,
                },
                order=order
            )
        ...
        # For cash, mark order as paid immediately and record transaction
        if payment_method == 'cash':
            ...
            # Release table if it exists
            if order.table:
                order.table.status = 'available'
                order.table.save()

            # Emit SSE so admin/orders UI updates immediately
            try:
