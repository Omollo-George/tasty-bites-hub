...
if (-not $SkipFrontend) {
    ...
    if (Test-Path "frontend/package.json") {
        ...
        Write-Red "✗ frontend/package.json not found"
        exit 1
    }
} else {
...
if (Test-Path "backend/tastybites/requirements.txt") {
    Push-Location backend/tastybites
    ...
    Write-Red "✗ backend/tastybites/requirements.txt not found"
    exit 1
}
