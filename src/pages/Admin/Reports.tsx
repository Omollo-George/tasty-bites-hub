...
const ReportCard = ({ label, value, valueClassName }: { label: string; value: string; valueClassName?: string }) => ( // Changed styling for ReportCard component.
  <div className="bg-slate-800 p-6 rounded-xl shadow-card border border-slate-700">
    <p className="text-sm text-slate-400">{label}</p>
    <h3 className={`text-3xl font-display ${valueClassName || 'text-slate-100'}`}>{value}</h3>
  </div>
)
...
const Reports: React.FC = () => {
  ...
  const [selectedPeriodType, setSelectedPeriodType] = useState<'day' | 'week' | 'month' | 'year'>('week');
  ...
  const fetchReport = async () => {
    ...
    try {
      const res = await fetch(getApiUrl(`/payments/reports/summary/?period_type=${selectedPeriodType}&date=${selectedDate}`), {
  ...
  const downloadReport = async () => {
    const token = getAdminToken()
    try {
      const url = getApiUrl(`/payments/reports/download/?period_type=${selectedPeriodType}&date=${selectedDate}`)
  ...
  const fetchWastage = async () => {
    const token = getAdminToken()
    try {
      const res = await fetch(getApiUrl(`/payments/reports/wastage/?period_type=${selectedPeriodType}&date=${selectedDate}`), {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (!res.headers.get("content-type")?.includes("application/json")) {
        const errorText = await res.text();
  ...
  const fetchMisc = async () => {
    const token = getAdminToken()
    try {
      const res = await fetch(getApiUrl(`/payments/reports/miscellaneous/?period_type=${selectedPeriodType}&date=${selectedDate}`), {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (!res.headers.get("content-type")?.includes("application/json")) {
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            onChange={(event) => setSelectedPeriodType(event.target.value as 'day' | 'week' | 'month' | 'year')}
          >
