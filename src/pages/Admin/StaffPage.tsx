...
import { ShoppingCart, UtensilsCrossed, Clock, CheckCircle, TrendingUp, Bell, LayoutGrid, Users, LogOut, Home, Monitor } from 'lucide-react';
import { getApiUrl } from '@/lib/api';
import { getAdminToken, isAdminSessionValid } from '@/lib/admin-session'; // Assuming getStaffRole exists in staff-session
import { getAuthToken } from '@/lib/auth';
import { useToast } from '@/hooks/use-toast'; // Assuming useToast is correctly imported
import { clearStaffSession, getStaffRole, getStaffName } from '@/lib/staff-session';
...
const StaffPage: React.FC = () => {
  ...
  const staffRole = getStaffRole(); // Get staff role
  ...
  useEffect(() => {
    const fetchTables = async () => {
      try {
        const res = await fetch(getApiUrl('/payments/pos/tables/'), { headers: { Authorization: `Bearer ${adminToken}` } });
    ...
    fetchTables();
  }, [authToken, location.pathname]); // Re-fetch tables if authToken changes or on route change (removed adminToken from dependency array)
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      ...
      try {
        // Assuming a backend endpoint that filters activities by role
        const res = await fetch(getApiUrl(`/payments/staff/activities/?role=${staffRole}`), {
          headers: { Authorization: `Bearer ${adminToken}` } // Use adminToken for auth, or staffToken if available
        });
        if (res.ok) {
          const data = await res.json();
          setActivities(data.activities || []); // Set activities from fetched data
        } else {
          ...
          // If it's a 404, it means the backend feature isn't implemented yet.
          // We log it to console but avoid showing a disruptive toast to the user every 10 seconds.
          if (res.status !== 404) {
            toast({
    ...
    fetchActivities();
    const interval = setInterval(fetchActivities, 10000); // Poll for new activities every 10 seconds
    return () => clearInterval(interval);
  }, [staffRole, authToken, toast]); // Added toast to dependency array as it's used inside

  const allStats = [
    { label: 'Orders Taken', value: '12', icon: ShoppingCart, color: 'text-blue-400', bg: 'bg-blue-500/10', roles: ['waiter', 'cashier', 'manager'] },
    { label: 'Tables Served', value: '8', icon: UtensilsCrossed, color: 'text-orange-400', bg: 'bg-orange-500/10', roles: ['waiter', 'manager'] },
    { label: 'Avg Prep Time', value: '15m', icon: Clock, color: 'text-emerald-400', bg: 'bg-emerald-500/10', roles: ['chef', 'manager'] },
    { label: 'Completed', value: '10', icon: CheckCircle, color: 'text-purple-400', bg: 'bg-purple-500/10', roles: ['waiter', 'cashier', 'chef', 'manager'] },
  ];

  const visibleStats = isAdmin 
    ? allStats 
    : allStats.filter(s => s.roles.includes(roleLower));
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {visibleStats.map((stat, idx) => (
            <div key={idx} className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex items-center gap-5 hover:border-slate-700 transition-colors">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex items-center gap-5 hover:border-slate-700 transition-colors">
            ...
            <div><p className="text-sm text-slate-400 font-medium">Available Tables</p><p className="text-2xl font-bold text-slate-100">{availableTables}</p></div>
          </div>
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex items-center gap-5 hover:border-slate-700 transition-colors">
            <div className="p-4 rounded-xl bg-orange-500/10 text-orange-400"><LayoutGrid size={28} /></div>
            <div><p className="text-sm text-slate-400 font-medium">Occupied Tables</p><p className="text-2xl font-bold text-slate-100">{occupiedTables}</p></div>
          </div>
        </div>
          ...
          <section className="bg-slate-900 border border-slate-800 p-8 rounded-3xl space-y-6">
            ...
            <div className="space-y-4">
              ...
              <ul className="space-y-3">
                {['Refill condiment stations', 'Check table 5 for cleanup', 'Restock beverage fridge', 'Update specials board'].map((task, i) => (
                  <li key={i} className="flex items-center gap-3 text-slate-300">
                    <input type="checkbox" className="rounded border-slate-700 bg-slate-800 text-orange-500 focus:ring-0 w-5 h-5 cursor-pointer" />
