...
const DEFAULT_MENU_ITEMS: RawMenuItem[] = [
  {
    id: 1,
...
const buildDefaultHomeData = (): HomeData => {
  const menuByCategory = DEFAULT_CATEGORIES.reduce<Record<string, RawMenuItem[]>>((acc, category) => {
    acc[category] = DEFAULT_MENU_ITEMS.filter((item) => item.category === category);
    return acc;
  }, {} as Record<string, RawMenuItem[]>);
